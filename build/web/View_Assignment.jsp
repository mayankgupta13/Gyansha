...
Document   : View_Assignment
    Created on : May 19, 2014, 4:38:25 PM
...
%>
	...
	else
	{
		...
		int assignment_id=Integer.parseInt(request.getParameter("f1"));
                session.setAttribute("Assi_Id", assignment_id);
		int chapter_id=Integer.parseInt(request.getParameter("f2"));
		
		Connection con = c.getConnection();
		Statement st=con.createStatement();
		Statement st1=con.createStatement();
		ResultSet rs=st.executeQuery("SELECT File_Name from create_assignment where Assignment_Id='"+assignment_id+"'");
		while(rs.next())
		{
			assignment_name=rs.getString(1);
...
<script type="text/javascript">
	...
	{			
        ...
        function more()
	{			
		var c=confirm("Are yoy sure to add more questions...?");
		if(c==true)
		{
			
			window.open("Assi_Questions.jsp","_Self");
			return true;
		}
		else
			return false;		
...
<div>
	...
	<div>
		<label class='g8-name'>Assignment Id :&nbsp;<%=assignment_id%></label>
	</div>
		&nbsp;
	<div>
	...
	<div id='contact-form-8'>
		...
		<% int a=1;%>
		<%ResultSet rs2=st.executeQuery("select Question_Id,Assignment_Question from Assignment_Questions where Assignment_Id='"+assignment_id+"'");
		while(rs2.next()){ int question_id=rs2.getInt("Question_Id");%>
	
		<tr><td><a href="Edit_Assignments.jsp?f1=<%=question_id%>&f2=<%=assignment_id%>&f3=<%=chapter_id%>">Edit</a></td></tr><tr>
			<td width="5%" align="center"><%=a %>.</td>
			<td colspan="8"><%=rs2.getString("Assignment_Question") %>
			</tr>	
			<tr>	
				<%ResultSet rs3=st1.executeQuery("select Options,Correct_Ans from Assignment_Options where Questions_Id="+question_id);
				while(rs3.next())
				{%>
					<%if(rs3.getBoolean("Correct_Ans")==true){%>
					<td width="3%" align="right"><img src="img/Yes.png"></td>
					<td><font color="GREEN"><%=rs3.getString("Options")%></font></td>
					<%
                                        }
					else{
                                        %>
					<td width="3%" align="right"><img src="img/no.png"></td>
					<td><%=rs3.getString("Options") %></td>
				<%}
			}a++;%>
			</tr>
			<tr>
		...
		<p class='contact-submit'>
			<input type="button" name='t6' id='g8-name' class='name' value="Finish" onclick="gonext()" class='pushbutton-wide'/>	&nbsp;
                        <input type="button" name='t6' id='g8-name' class='name' value="Add Question"  class='pushbutton-wide' onclick="more()"/>
		</p>
	</form>
	</div>
	</div>
